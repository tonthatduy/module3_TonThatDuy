...
<form method="post" action="students?action=update">
  ...
  <fieldset>
    ...
    <table>
      <tr>
        <td>Name:</td>
        <td><input type="text" name="nameStudent" id="nameStudent" value="${student.nameStudent}" required></td>
      </tr>
      <tr>
        <td>Class:</td>
        <td>
          <select name="idClass" id="idClass" required>
            ...
            <c:forEach items="${classes}" var="classItem">
              <option value="${classItem.idClass}" <c:if test="${classItem.idClass == student.idClass}">selected</c:if>>${classItem.nameClass}</option>
            </c:forEach>
          </select>
        </td>
      </tr>
