...
INSERT INTO dich_vu (
ten_dich_vu, dien_tich, chi_phi_thue, so_nguoi_toi_da, tieu_chuan_phong, 
mo_ta_tien_nghi_khac, dien_tich_ho_boi, so_tang, ma_kieu_thue, ma_loai_dich_vu) VALUES
...
select *
from nhan_vien where (ho_ten like 'H%' or ho_ten like 'T%' or ho_ten like 'K%') and char_length(ho_ten) <= 15;

-- 3.Hiển thị thông tin của tất cả khách hàng
--  có độ tuổi từ 18 đến 50 tuổi và có địa chỉ ở “Đà Nẵng” hoặc “Quảng Trị”.

select *
from khach_hang where (TIMESTAMPDIFF(year,ngay_sinh,curdate())) between 18 and 50 
...
count(distinct ma_khach_hang) as so_khach_dat_phong
from hop_dong hd
where year(hd.ngay_lam_hop_dong) = 2021
group by thang
order by thang;
...
having so_luong_dich_vu_di_kem = (
	select max(so_luong)
    from (
		select sum(hdct.so_luong) as so_luong
